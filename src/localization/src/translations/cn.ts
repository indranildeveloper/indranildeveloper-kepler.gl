...
export default {
  ...
  modal: {
    ...
    addStyle: {
      ...
      publishSubtitle7: '。* indranil-kepler.gl 是一个客户端应用程序，数据保留在您的浏览器中。',
    ...
    shareMap: {
      ...
      shareDisclaimer:
        'indranil-kepler.gl 将创建的地图存储在个人云存储中，因此只有知道 URL 的人才能访问地图及其数据。' +
        '可以随时使用个人云存储帐户编辑/删除数据文件。',
      gotoPage: '跳转到indranil-kepler.gl的{currentProvider}页面'
    },
    statusPanel: {
      mapUploading: '地图上传中',
      error: '错误'
    ...
    exportMap: {
      ...
      json: {
        configTitle: '地图配置',
        configDisclaimer:
          '地图配置将包含在 Json 文件中。如果您在自己的应用程序中使用 indranil-kepler.gl。您可以复制此配置并将其传递给',
        selection:
          '将当前地图数据和配置导出到单个 Json 文件中。稍后您可以通过将此文件上传到 indranil-kepler.gl 来打开同一张地图。',
        disclaimer:
    ...
    iconInfo: {
      ...
      code: '图标',
      description2: '时，indranil-kepler.gl 会自动为你创建一个图标层。',
      example: '例:',
      icons: '图标一览'
    },
    storageMapViewer: {
    ...
    loadStorageMap: {
      back: '返回',
      goToPage: '跳转到 indranil-kepler.gl 的 {displayName} 页面',
      storageMaps: '存储 / 地図',
  ...
  fileUploader: {
    ...
    disclaimer:
      '* indranil-kepler.gl 在客户端上工作。数据仅保留在您自己的设备/浏览器中。' +
      '没有信息或地图数据被发送到任何服务器。',
    configUploadMessage:
