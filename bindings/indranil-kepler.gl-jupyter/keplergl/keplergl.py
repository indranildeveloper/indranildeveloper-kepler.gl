...
documentation = 'https://docs.indranil-kepler.gl/docs/keplergl-jupyter'
...
def _df_to_dict(df):
    ''' Create an input dict for indranil-kepler.gl using a DataFrame object

    Inputs:
    - df: a DataFrame object

    Returns:
    - dictionary: a dictionary variable that can be used in indranil-kepler.gl
...
def _df_to_arrow(df: pd.DataFrame):
    ''' Create an arrow base64string for indranil-kepler.gl using a DataFrame object

    Inputs:
    - df: a DataFrame object

    Returns:
    - string: a base64 string that can be used in indranil-kepler.gl
    '''
...
def _gdf_to_dict(gdf):
    ''' Create an input dict for indranil-kepler.gl using a GeoDataFrame object
...
def _gdf_to_arrow(gdf):
    ''' Create an arrow base64string for indranil-kepler.gl using a GeoDataFrame object'''
    # reproject to 4326 if needed
    if gdf.crs and not gdf.crs == 4326:
        gdf = gdf.to_crs(4326)
