...
import {addDataToMap, ActionTypes} from '@indranil-kepler.gl/actions';
...
class KeplerGlJupyter {
  ...
  onDataChange(that) {
    log('indranil-kepler.gl onDataChange');
    const data = getData(that);
  ...
  onConfigChange(that) {
    log('indranil-kepler.gl onConfigChange');
    const config = getConfig(that);
