...
import {processCsvData, processGeojson, processArrowBatches} from '@indranil-kepler.gl/processors';
import log from '../log';
...
function processReceivedData({data, info}) {
  ...
  try {
    ...
    console.log(
      `indranil-kepler.gl fails to parse data, detected data
...
function processDataFrame(data) {
  ...
  // indranil-kepler.gl will detect field types
  return {fields, rows};
}
