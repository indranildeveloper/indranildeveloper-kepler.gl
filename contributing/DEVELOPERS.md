...
- [Developing indranil-kepler.gl Website](./#develop-the-kepler-gl-website)
- [Publish the website](./#publish-the-website)
- [Publish a new version](./#publish-kepler-gl-package-to-npm)

## Development Setup

This document describes how to set up your development environment to build and test indranil-kepler.gl, and
explains the basic mechanics of using `git`, `node`, `yarn`.

### Installing Dependencies

Before you can build indranil-kepler.gl, you must install and configure the following dependencies on your
machine:
...
#### Fork indranil-kepler.gl Repo

If you plan to contribute code to indranil-kepler.gl, you must have a [GitHub account](https://github.com/signup/free) so you can push code and open Pull Requests in the [GitHub Repository][github]. You must [fork](http://help.github.com/forking) the
[main indranil-kepler.gl repository][github] to [create a Pull Request][github-pr].

#### Developing indranil-kepler.gl

To develop features, debug code, run tests, we use webpack to start a local web server and serve the indranil-kepler.gl demo app from the src directory.

```bash
# Clone your indranil-kepler.gl fork repository:
git clone git@github.com:<github username>/indranil-kepler.gl.git

# Go to the indranil-kepler.gl directory:
cd indranil-kepler.gl

# Add the main indranil-kepler.gl repository as an upstream remote to your repository:
git remote add upstream "git@github.com:keplergl/indranil-kepler.gl.git"
```
...
# Start the indranil-kepler.gl demo app
yarn start
```

An demo app will be served at `http://localhost:8080/`

This is the demo app we hosted on [http://indranil-kepler.gl/#/demo][demo-app]. By default, it serves non-minified source code inside the src directory.
...
// load deck.gl src from the deck.gl folder parallel to indranil-kepler.gl
...
we use the git commit messages to **generate the indranil-kepler.gl change log**.
...
The indranil-kepler.gl project uses [jsdoc](http://usejsdoc.org/)
...
and misceallenous pages) that live inside the indranil-kepler.gl repository as markdown files.

This means that since we generate the documentation from the source code, we can easily provide
version-specific documentation by simply checking out a version of indranil-kepler.gl and running the build.
...
## Develop The indranil-kepler.gl Website
...
[Netlify](https://www.netlify.com/) is used to support indranil-kepler.gl demo website.
...
A new production version of indranil-kepler.gl website is automatically created and deployed every time a PR is merged onto master.
...
### Publish indranil-kepler.gl package to NPM
...
In order to publish a new version of indranil-kepler.gl a developer must perform the following steps:
...
- Update each of the example folder package.json indranil-kepler.gl dependency with the newer. To update all examples, run
...
indranil-kepler.gl documentation is hosted on [gitbook](https://kepler-gl.gitbook.io/kepler-gl/). For more information [read here](https://docs.gitbook.com/)
...
The above file is used by Gitbook to generate the doc navigation visible on the left-hand side of indranil-kepler.gl doc website.
...
- Update doc files in the indranil-kepler.gl repo. Follow the PR flow like any other changes
- Update documentation directly on Gitbook.

For both scenarios, changes will be propagated from one system to the other and vice versa. When updating Gitbook, a new git commit will be push to the indranil-kepler.gl master branch.
