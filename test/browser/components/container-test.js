...
import Container, {ERROR_MSG} from '@indranil-kepler.gl/components';
import {DEFAULT_MAPBOX_API_URL} from '@indranil-kepler.gl/constants';
...
test('Components -> Container -> Mount with mint:true', t => {
  ...
  // mount without id or a indranil-kepler.gl state
  mount(
  ...
  // mount with indranil-kepler.gl state
  ...
  let expectedActions0 = {
    type: '@@indranil-kepler.gl/REGISTER_ENTRY',
    payload: {
      id: 'map',
      mint: true,
  ...
  // unmount
  wrapper.unmount();
  expectedActions0 = {type: '@@indranil-kepler.gl/DELETE_ENTRY', payload: 'milkshake'};
...
test('Components -> Container -> Mount then rename', t => {
  ...
  const expectedActions1 = {
    type: '@@indranil-kepler.gl/RENAME_ENTRY',
  ...
  const expectedActions2 = {type: '@@indranil-kepler.gl/DELETE_ENTRY', payload: 'milkshake-2'};
