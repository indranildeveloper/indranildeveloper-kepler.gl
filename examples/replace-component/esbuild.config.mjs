...
const RESOLVE_LOCAL_ALIASES = {
  ...
  // indranil-kepler.gl and loaders.gl need to use same apache-arrow
...
const config = {
  ...
  plugins: [
    // automatically injected indranil-kepler.gl package version into the bundle
    replace({
...
function addAliases(externals, args) {
  ...
  if (useLocalDeck || useRepoDeck) {
    ...
    // if env.deck_src Load deck.gl from deck.gl/modules/main/src folder parallel to indranil-kepler.gl
    ...
    // if env.deck_src Load @deck.gl modules from  deck.gl/modules folder parallel to indranil-kepler.gl
    ...
    ['luma.gl', 'probe.gl', 'loaders.gl'].forEach(name => {
      ...
      // if env.deck_src Load ${name} from deck.gl/node_modules folder parallel to indranil-kepler.gl
      ...
      // if env.deck_src Load @${name} modules from deck.gl/node_modules/@${name} folder parallel to indranil-kepler.gl`
  ...
  if (args.includes('--start')) {
    await esbuild
      ...
      .then(async ctx => {
        ...
        console.info(
          `indranil-kepler.gl demo app running at ${`http://localhost:${port}`}, press Ctrl+C to stop`
        );
