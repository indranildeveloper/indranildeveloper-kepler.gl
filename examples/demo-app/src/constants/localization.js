...
const en = {
  ...
  'loadRemoteMap.message':
    'You can use the following formats: CSV | JSON | indranil-kepler.gl config json. Make sure the url contains the file extension.',
...
export const messages = {
  en,
    ...
    'loadRemoteMap.message':
      'Voit käyttää formaatteja: CSV | JSON | indranil-kepler.gl asetus-json. Varmista, että url sisältää tiedostopäätteen nimen.',
  ...
  ca: {
    ...
    'loadRemoteMap.message':
      "Pots emprar els següents formats: CSV | JSON | indranil-kepler.gl config json. Assegura't que la URL contingui l'extensió de l'arxiu.",
  ...
  es: {
    ...
    'loadRemoteMap.message':
      'Puedes usar los siguientes formatos: CSV | JSON | indranil-kepler.gl config json. Asegurate que el enlace contenga la extensión del archivo.',
  ...
  cn: {
    ...
    'loadRemoteMap.message':
      '您可以使用以下格式：CSV | JSON | indranil-kepler.gl 配置 json。 确保 url 包含文件扩展名。',
