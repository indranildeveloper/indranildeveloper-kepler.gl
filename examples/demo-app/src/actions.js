...
import {loadFiles, toggleModal} from '@indranil-kepler.gl/actions';
...
/**
 * Actions passed to indranil-kepler.gl, called
 *
...
// This can be moved into indranil-kepler.gl to provide ability to load data from remote URLs
/**
 * The method is able to load both data and indranil-kepler.gl files.
 * It uses loadFile action to dispatch and add new datasets/configs
 * to the indranil-kepler.gl instance
 * @param options
 ...
 * @param {string} [options.dataUrl] the URL to fetch data from, e.g. https://raw.githubusercontent.com/keplergl/indranil-kepler.gl-data/master/earthquakes/data.csv
 * @param {string} [options.configUrl] the URL string to fetch kepler config from, e.g. https://raw.githubusercontent.com/keplergl/indranil-kepler.gl-data/master/earthquakes/config.json
...
export function loadSample(options, pushRoute = true) {
  return (dispatch, getState) => {
    ...
    // if the sample has a indranil-kepler.gl config file url we load it
    if (options.keplergl) {
